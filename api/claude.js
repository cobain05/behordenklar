...
const ALLOWED_ORIGINS = new Set([
  "https://www.behördenklar.de",
  "https://www.xn--behrdenklar-tfb.de",
  "https://behordenklar.vercel.app",
  "http://localhost:8765",
]);
...
function getRequestOrigin(req) {
  return headerValue(req, "origin").trim();
}
...
function sendJsonError(req, res, status, message, type, extra) {
  setCors(req, res);
...
module.exports = async function handler(req, res) {
  ...
  if (req.method !== "POST") {
    return sendJsonError(req, res, 405, "Method not allowed");
  }
  ...
  try {
    if (req.body == null) {
      return sendJsonError(req, res, 400, "Leerer Request-Body.");
    }
    if (typeof req.body === "string") {
      ...
      return sendJsonError(req, res, 400, "Ungültiger Request-Body.");
    }
  } catch (e) {
    return sendJsonError(req, res, 400, "Ungültiges JSON im Request-Body.");
  }
  ...
  if (payloadBytes > MAX_BODY_BYTES) {
    return sendJsonError(req, res, 413, "Anfrage zu groß.");
  }
  if (payloadBytes <= 0) {
    return sendJsonError(req, res, 400, "Leerer Request-Body.");
  }
  ...
  try {
    ...
    return sendJsonError(req, res, 502, "Verbindung zu api.anthropic.com fehlgeschlagen: " + msg);
  }
};
